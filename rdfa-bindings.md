...
{.ednote ...} This is an **early draft** of a standard documenting the
...
This standard depends on the [Citation Elements] standard.  To be
*conformant* with this standard, an application *must* also be
conformant with [Citation Elements].  Some words and phrases defined in
that standard are used here without further definition.

{.note} Readers are advised to read at least the introduction to
[Citation Elements] before reading this standard.
...
{.ednote} [HTML+RDFa] adds special handling for the `datetime` attribute
and `time` element to sequence step 11.  Should we too?

## Shorthand IRIs

The [Citation Elements] standard makes heavy use of IRIs as identifiers,
as does RDFa.  In particular, the `datatype`, `property` and `typeof`
...
defined in §3.4.1 of [Citation Elements] to disambiguate these cases.
...
However, [Citation Elements] recommends that the first *string* in the
...
{.ednote} Consider adding support for `@datetime` and `<date>` if the
*host language* is HTML.
...
{.ednote} The [Citation Elements] standard does not currently define a
